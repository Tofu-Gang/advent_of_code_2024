...
def puzzle_01() -> int:
    ...
    :return: Your actual left and right lists contain many location IDs. What is
    the total distance between your lists?
    """

    with open(INPUT_FILE_PATH, "r") as f:
        lines = f.readlines()
        left = sorted(list(int(line.split()[0]) for line in lines))
        right = sorted(list(int(line.split()[1]) for line in lines))
...
def puzzle_02() -> int:
    ...
    :return: Once again consider your left and right lists. What is their
    similarity score?
    """
    ...
    with open(INPUT_FILE_PATH, "r") as f:
        ...
        left = sorted(list(int(line.split()[0]) for line in lines))
        right = sorted(list(int(line.split()[1]) for line in lines))
        return sum(locationID * right.count(locationID) for locationID in left)
